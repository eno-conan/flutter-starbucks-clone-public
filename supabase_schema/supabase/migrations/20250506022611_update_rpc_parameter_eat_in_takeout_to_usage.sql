drop function if exists "public"."create_order_with_details"(p_order_id character varying, p_store_number character varying, p_eat_in_takeout smallint, p_order_type smallint, p_pickup_number character varying, p_price_without_tax integer, p_price_with_tax integer, p_payment_method character varying, p_order_details jsonb);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.create_order_with_details(p_order_id character varying, p_store_number character varying, p_usage smallint, p_order_type smallint, p_pickup_number character varying, p_price_without_tax integer, p_price_with_tax integer, p_payment_method character varying, p_order_details jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$DECLARE
